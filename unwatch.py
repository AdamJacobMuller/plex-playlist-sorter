...
for video in videos:
    ...
    res = requests.get("%s/:/unscrobble?identifier=com.plexapp.plugins.library&key=%s" % (args.address, video.attrib['ratingKey']))

    print "%s %s[%s] = %s" % (video.attrib.get("originallyAvailableAt"), name, video.attrib['playlistItemID'], str(res))
